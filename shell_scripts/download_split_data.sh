...
list=`ls $storage/DGS_corpus | ruby -e 'puts STDIN.readlines.shuffle(random: Random.new(42))'`

cp -r `echo "$list" | head -$devindex | awk -v var="$storage" '{print var"/DGS_corpus/"$1}'` $storage/DGS_dev/

cp -r `echo "$list" | tail -$trainindex | awk -v var="$storage" '{print var"/DGS_corpus/"$1}'` $storage/DGS_train/

devindex=`echo $devindex | awk 'END {print $1+1}'`

cp -r `echo "$list" | head -$testindex | tail -$devindex |  awk -v var="$storage" '{print var"/DGS_corpus/"$1}'` $storage/DGS_test/
