...
src=sign
trg=de
data=$3/Extracted_data_timestamps
...
num_threads=1
model_name=frame_embeddings_sign_de
...
OMP_NUM_THREADS=$num_threads python -m sockeye.train \
      ...
      -s $data/train.preprocessed.$src \
      -t $data/train.preprocessed.$trg \
      -sfe $data/train.preprocessed.time \
	  -vs $data/dev.preprocessed.$src \
      -vt $data/dev.preprocessed.$trg \
      -vsfe $data/dev.preprocessed.time \
      --max-updates 1001000 \
      --seed=1 \
      --batch-type=word \
      --batch-size=400 \
      ...
      --transformer-positional-embedding-type 'frames_source' \
      --label-smoothing 0.1 \
